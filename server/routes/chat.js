...
router.post('/chat', async (req, res) => {
  ...
  if (!openai) {
    // Fallback to local AI when no API key is set
...
// Analyze uploaded image with Vision AI
...
router.post('/conversations', (req, res) => {
  ...
  `).run(id, userId || null, title || 'New Conversation');

  res.json({ success: true, conversation: { id, title: title || 'New Conversation' } });
});
...
// Get all AI conversations for admin
...
// Get all image analyses for admin
router.get('/admin/image-analyses', (req, res) => {
  const { db } = req;
...
router.post('/upload-image', (req, res) => {
  // For now, accept base64 data URLs and store as-is
...
// Helper: Local vision analysis fallback
